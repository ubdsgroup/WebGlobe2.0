...
define(['knockout', 'model/Config', 'model/Constants', 'worldwind'],
    function (ko,
        ...
        LayerManager.prototype.addWidgetLayer = function (layer, options) {
            ...
            LayerManager.applyOptionsToLayer(layer, options, constants.LAYER_CATEGORY_BACKGROUND);
         ...
         * @param {Array} wmsCapsDoc
         * @returns {LayerManager_L17.LayerManager.prototype.loadServerCapabilites.server}
         */
...
//                        this.timeSeriesPlayer.layer = null;
                ...
                if (category === constants.LAYER_CATEGORY_BASE) {
                    ...
                    this.addOverlayLayer(layer,{hideInMenu: true,
		                    enabled: true});
        ...
        LayerManager.prototype.removeLayer = function (layerCaps) {
            ...
            var layer;
            var e = -1;
            var r = this.globe.wwd.layers.length;
            for (var i = 0; i < r; i++) {
                var l = this.globe.wwd.layers[i];
                if(l.config && l.config.title && layerCaps.title){
                    if (l.config.title === layerCaps.title) {
                        layer = l;
                        e = i;
                        break;
                    }
            ...
            // remove layer from appropriate category
            if(layer){
                if (layer.category === constants.LAYER_CATEGORY_OVERLAY) {
                    this.removeOverlayLayer(layer);
                }
            }
           
            this.globe.wwd.removeLayer(e);
        ...
        LayerManager.prototype.removeOverlayLayer = function (layer) {
            this.overlayLayers.pop(LayerManager.createLayerViewModel(layer));
        };
